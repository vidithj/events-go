...
func (rr *RoundRobin) Update(ev sd.Event) {
	...
	for _, i := range insts {
		...
		if err := newDB.eventsDB.Client().Authenticate(context.Background(), couchdb.BasicAuth(rr.dbusername, rr.dbpassword)); err != nil {
			continue
		}
		newDB.eventsDB.Client().CreateDB(context.Background(), rr.dbName, nil)
